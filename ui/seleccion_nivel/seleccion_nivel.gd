...
func _ready() -> void:
	var progreso: Dictionary = GLOBAL.obtener_progreso_nivel(id_nivel)
	nivel_boton.text = str(id_nivel)
	estrellas_contenedor.pintar_estrellas(int(progreso.estrellas))

	if id_nivel_anterior > 0:
		var progreso_nivel_anterior: Dictionary = GLOBAL.obtener_progreso_nivel(id_nivel_anterior)
...
func _on_nivel_boton_pressed() -> void:
	var pantalla_nivel: String = "NIVEL_" + str(id_nivel)
